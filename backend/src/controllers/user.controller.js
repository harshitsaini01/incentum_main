...
const ADMIN_CREDENTIALS = process.env.ADMIN_CREDENTIALS 
  ...
  : [{ phoneNumber: "9876543210", password: "Incentum!Business1" }]; // Default admin credentials
...
const registerUser = asyncHandler(async (req, res) => {
  try {
    ...
    if (existingUserByPhone) {
      throw new ApiError(409, "User with this phone number already exists! Please login.");
    }
...
const loginUser = asyncHandler(async (req, res) => {
  try {
    ...
    if (!user) {
      throw new ApiError(401, "No account found with this phone number. Please register first.");
    }
