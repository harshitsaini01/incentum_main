    ...
    switch (step) {
      ...
      case 4: // Documents - handled separately in uploadDocument
        ...
        if (stepData.coApplicants) {
          console.log('Processing co-applicants in saveStepData:', stepData.coApplicants);
          ...
          const finalCleanedCoApplicants = cleanedCoApplicants.map(coApplicant => {
            ...
            const result = {
              ...
              personalDetails: cleanedPersonalDetails,
              employmentDetails: cleanedEmploymentDetails,
              documents: coApplicant.documents || [] // Preserve documents
            };
            console.log('Co-applicant after processing:', { 
              name: result.personalDetails?.fullName || result.personalDetails?.full_name,
  ...
  upload.single('document')(req, res, async function (err) {
    ...
    try {
      ...
      res.status(200).json({
        ...
        data: {
          fileUrl,
          filePath: relativePath, // Send relative path for database storage
          fileName: req.file.originalname,
          documentType
        }
      });
