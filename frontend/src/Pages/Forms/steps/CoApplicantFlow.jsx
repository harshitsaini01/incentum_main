...
const CoApplicantFlow = ({ onComplete, onCancel, existingData = null, title = "Add Co-Applicant", loanType = null, applicationId = null, coApplicantIndex = 0 }) => {
  ...
  const handleDocumentUpload = async (documentType, file) => {
    if (!file || !applicationId) {
      console.error('Missing file or applicationId:', { file: !!file, applicationId });
      return;
    }
    ...
    try {
      ...
      formData.append('applicantType', `coApplicant${coApplicantIndex + 1}`);

      console.log('Uploading co-applicant document:', { documentType, applicationId, coApplicantIndex: coApplicantIndex + 1 });

      const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8080'}/api/multi-step-form/upload-document`, {
        method: 'POST',
        body: formData,
        credentials: 'include'
      });
      ...
      if (data.success) {
        // Store the uploaded document info in co-applicant documents
        setCoApplicantData(prev => ({
          ...prev,
          documents: {
            ...prev.documents,
            [documentType]: {
              ...
              filePath: data.data.filePath || data.data.fileUrl, // Use relative path if available
              uploadedAt: new Date().toISOString()
            }
          }
        }));

        console.log(`Co-applicant ${documentType} uploaded successfully to server`);
      } else {
  ...
  const handleComplete = () => {
    // Convert documents object to array format for backend
    const documentsArray = Object.entries(coApplicantData.documents)
      .filter(([key, value]) => value && value.fileName) // Only include documents that have been uploaded
      .map(([key, value]) => ({
        documentType: key,
    ...
    console.log('Co-applicant data being sent:', processedData);
