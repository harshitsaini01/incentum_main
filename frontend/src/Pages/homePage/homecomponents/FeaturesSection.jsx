...
const FeaturesSection = () => {
  ...
  return (
    <div id="features-section" className="relative bg-features-gradient min-h-screen py-20 pt-32">
      ...
      {/* Enhanced Navigation Tabs - Beautiful Mobile Slider */}
      <div className="relative z-20 bg-banking-navy/90 backdrop-blur-md border-b border-banking-accent/30 opacity-100 animate-slide-down" style={{animationDelay: '0.9s'}}>
        <div className="container mx-auto px-4 py-6">
          <div className="flex justify-center">
            {/* Desktop Navigation */}
            <div className="hidden md:flex bg-banking-dark/80 backdrop-blur-md rounded-2xl p-3 space-x-3 border border-banking-accent/40 shadow-2xl shadow-banking-accent/10 animate-scale-in">
              {loanTypes.map((loan, index) => (
                <button
                  key={loan.id}
                  className={`flex items-center space-x-2 px-6 lg:px-8 py-4 rounded-xl text-sm lg:text-base font-semibold whitespace-nowrap transition-all duration-300 animate-fade-in ${
                    activeTab === loan.id
            ...
            {/* Mobile Navigation - Beautiful Slider */}
            <div className="md:hidden w-full">
              <div className="relative bg-banking-dark/80 backdrop-blur-md rounded-2xl p-4 border border-banking-accent/40 shadow-2xl shadow-banking-accent/10">
                
                {/* Slider Header with Instructions */}
                <div className="flex items-center justify-between mb-4">
                  <h3 className="text-white text-sm font-semibold">Choose Loan Type</h3>
                  <div className="flex items-center space-x-2 text-banking-accent text-xs">
                    <div className="flex items-center space-x-1 bg-banking-accent/10 px-2 py-1 rounded-full">
                ...
                {/* Slider Container */}
                <div 
                  className="flex overflow-x-auto space-x-2 pb-3 mobile-slider" 
                  style={{
                ...
                >
                  {loanTypes.map((loan) => (
                    <button
                      key={loan.id}
                      className={`flex-shrink-0 flex flex-col items-center space-y-2 px-3 py-3 rounded-xl text-xs font-semibold transition-all duration-300 min-w-[85px] scroll-snap-align-center ${
                        activeTab === loan.id
                          ? `text-white bg-gradient-to-br from-banking-accent via-banking-lightBlue to-banking-skyBlue shadow-xl border-1 border-banking-accent/60 transform scale-10`
                          : "text-gray-200 bg-banking-navy/60 hover:bg-banking-navy/80 border-2 border-transparent hover:border-banking-accent/30 hover:text-white"
                      }`}
                      onClick={() => {
                        const section = document.getElementById(loan.id);
                        if (section) {
                    ...
                    >
                      <div className={`w-6 h-6 rounded-full flex items-center justify-center text-sm transition-all duration-300 ${
                        activeTab === loan.id 
                          ? 'bg-white/25 shadow-lg animate-pulse' 
                          : 'bg-banking-accent/20 hover:bg-banking-accent/30'
                      }`}>
                        {loan.icon}
                      </div>
                      <span className="text-center leading-tight font-medium text-[12px]">{loan.label}</span>
                      {activeTab === loan.id && (
                        <div className="w-4 h-0.5 bg-white rounded-full opacity-90 animate-pulse"></div>
                      )}
                    </button>
                  ))}
                </div>
                ...
                <div className="absolute left-4 top-16 bottom-16 w-6 bg-gradient-to-r from-banking-dark/80 to-transparent pointer-events-none rounded-l-xl"></div>
                <div className="absolute right-4 top-16 bottom-16 w-6 bg-gradient-to-l from-banking-dark/80 to-transparent pointer-events-none rounded-r-xl"></div>
                ...
                <div className="flex items-center justify-center mt-4 space-x-3">
                    ...
                    {loanTypes.map((loan) => (
                      <div
                        ...
                        className={`rounded-full transition-all duration-300 cursor-pointer shadow-sm ${
                          activeTab === loan.id 
                            ? 'bg-gradient-to-r from-banking-accent to-banking-lightBlue w-8 h-2 shadow-lg shadow-banking-accent/30' 
                            : 'bg-gray-500 w-2 h-2 hover:bg-gray-400 hover:scale-110'
                        }`}
                        onClick={() => {
                          const section = document.getElementById(loan.id);
                          if (section) {
                  ...
                  <div className="ml-3 bg-banking-accent/10 px-2 py-1 rounded-full">
                    <div className="text-xs text-banking-accent font-semibold flex items-center space-x-1">
                      <span>{loanTypes.findIndex(loan => loan.id === activeTab) + 1}</span>
                      <span className="text-banking-accent/60">/</span>
                      <span>{loanTypes.length}</span>
                    </div>
                  </div>
                </div>
              </div>
      ...
      <div className="relative z-10 mt-24 opacity-100">
        <div className="text-center mb-16">
          ...
          <h3 className="font-display text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-white mb-4 sm:mb-6">
          ...
          <p className="font-banking text-base sm:text-lg md:text-xl text-gray-200 max-w-3xl mx-auto leading-relaxed px-4">
          ...
          {/* Success Stats */}
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mt-8 sm:mt-12 max-w-4xl mx-auto px-4">
            <div className="bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-banking-accent/30 hover:border-banking-accent/50 transition-all duration-300 group">
              <div className="text-2xl sm:text-3xl font-bold text-banking-accent mb-1 sm:mb-2 group-hover:scale-110 transition-transform duration-300">4.9★</div>
              <div className="text-gray-200 text-xs sm:text-sm">Customer Rating</div>
            </div>
            <div className="bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-banking-accent/30 hover:border-banking-accent/50 transition-all duration-300 group">
              <div className="text-2xl sm:text-3xl font-bold text-banking-accent mb-1 sm:mb-2 group-hover:scale-110 transition-transform duration-300">₹50Cr+</div>
              <div className="text-gray-200 text-xs sm:text-sm">Dreams Funded</div>
            </div>
            <div className="bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl p-4 sm:p-6 border border-banking-accent/30 hover:border-banking-accent/50 transition-all duration-300 group">
              <div className="text-2xl sm:text-3xl font-bold text-banking-accent mb-1 sm:mb-2 group-hover:scale-110 transition-transform duration-300">98%</div>
              <div className="text-gray-200 text-xs sm:text-sm">Success Rate</div>
            </div>
          </div>
        </div>
        ...
        <div className="relative w-full max-w-6xl mx-auto px-4">
                ...
                <div
                  className={`relative transition-all duration-500 ${
                    activeIndex === index 
                      ...
                      : "opacity-70 scale-90 sm:scale-90"
                  }`}
                >
                  <div className="bg-white/95 backdrop-blur-sm rounded-xl sm:rounded-2xl p-3 sm:p-4 border border-banking-accent/30 hover:border-banking-accent/50 transition-all duration-300 shadow-xl group">
                    <img
                      src={img}
                      alt={`Success Story ${index + 1}`}
                      className="w-full h-[50vh] min-h-96 sm:h-72 md:h-80 lg:h-96 object-cover rounded-lg sm:rounded-xl group-hover:scale-105 transition-transform duration-300"
                    />
                    
                    {/* Success Story Overlay */}
                    <div className="absolute bottom-3 sm:bottom-4 left-3 sm:left-4 right-3 sm:right-4 bg-banking-dark/90 backdrop-blur-sm rounded-md sm:rounded-lg p-2 sm:p-3 text-white opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <div className="flex items-center space-x-1 sm:space-x-2 mb-1">
                        <div className="w-1.5 h-1.5 sm:w-2 sm:h-2 bg-banking-accent rounded-full animate-pulse"></div>
                        <span className="text-xs font-semibold text-banking-accent">SUCCESS STORY</span>
                      </div>
                      <p className="text-xs sm:text-sm font-medium">Loan Approved Successfully</p>
                      <p className="text-xs text-gray-300">Fast • Secure • Reliable</p>
                    </div>
                  </div>
                  
                  {activeIndex === index && (
                    <div className="absolute inset-0 bg-gradient-to-t from-banking-accent/20 to-transparent rounded-xl sm:rounded-2xl pointer-events-none"></div>
                  )}
                </div>
              </SwiperSlide>
            ))}
          ...
          <div className="text-center mt-8 sm:mt-12">
            <div className="bg-white/10 backdrop-blur-md rounded-xl sm:rounded-2xl p-6 sm:p-8 border border-banking-accent/30 max-w-2xl mx-auto">
              <h4 className="font-display text-lg sm:text-xl md:text-2xl font-bold text-white mb-3 sm:mb-4">
                Ready to Write Your <span className="text-banking-accent">Success Story?</span>
              </h4>
              <p className="text-gray-200 mb-4 sm:mb-6 leading-relaxed text-sm sm:text-base px-2">
                Join our community of successful borrowers and turn your financial dreams into reality.
              </p>
              <Link
                to="/loan-application"
                className="inline-flex items-center space-x-2 bg-gradient-to-r from-banking-accent to-banking-lightBlue text-white font-semibold px-6 sm:px-8 py-3 sm:py-4 rounded-lg sm:rounded-xl shadow-lg hover:shadow-2xl transform hover:scale-105 transition-all duration-300 text-sm sm:text-base"
              >
