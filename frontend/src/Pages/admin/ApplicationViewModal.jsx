...
const ApplicationViewModal = ({ isOpen, onClose, applicationId, onEdit }) => {
  ...
  const handleDownload = async () => {
    try {
      const response = await axios.get(`/admin/applications/${applicationId}/download`, {
        responseType: 'blob',
        withCredentials: true
      });
  ...
  const handleDocumentDownload = async (documentType, applicantIndex = 0, isCoApplicantDoc = false, docFileName = null) => {
    try {
      ...
      const url = `${axios.defaults.baseURL}/admin/applications/${applicationId}/documents/${documentType}/${applicantIndex}`;
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center space-x-3">
              <button
                ...
                className="relative group bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-600 hover:to-amber-700 text-white px-5 py-2 rounded-lg flex items-center space-x-2 transition-all duration-300 shadow-lg hover:shadow-xl transform hover:scale-105"
              >
